...
export function createAPIError(data: any, response?: Response): APIError {
  ...
  if (data["Note"] && data["Note"].includes("call frequency")) {
    return {
      type: "RATE_LIMIT",
      message: "เกินขีดจำกัดการเรียก API (5 ครั้ง/นาที) กรุณารอสักครู่แล้วลองใหม่",
  ...
  if (data["Note"] && data["Note"].includes("premium")) {
    return {
      type: "RATE_LIMIT",
      message: "เกินขีดจำกัดการเรียก API รายวัน (500 ครั้ง/วัน) กรุณาลองใหม่พรุ่งนี้",
...
// Get real-time quote
export async function getGlobalQuote(symbol: string): Promise<AlphaVantageQuote> {
...
export function calculateSupportResistance(priceData: { [date: string]: { "4. close": string } }) {
